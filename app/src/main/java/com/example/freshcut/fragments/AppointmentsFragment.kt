...
class AppointmentsFragment : Fragment() {
    ...
    override fun onResume() {
        super.onResume()
        view?.let { activeView ->
            activeView.findViewById<RelativeLayout>(R.id.loadingPanel).visibility = View.VISIBLE
            checkPermissionsAndLoad(activeView)
        }
    }
    ...
    fun checkPermissionsAndLoad(view: View) {
        ...
        if (currentUserId != null) {
            DatabaseManager.isUserManager(
                ...
                onFailure = {
                    // Failsafe: if role check fails, default to customer
                    loadAppointments(view, false)
                }
            )
        } else {
            // Not logged in failsafe
    ...
    private fun loadAppointments(view: View, isManager: Boolean) {
        // Pass the isManager variable to our newly updated DatabaseManager function!
        DatabaseManager.getFutureAppointments(
            isManager = isManager,
